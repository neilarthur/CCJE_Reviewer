...
if (isset($_POST['save'])) {
  ...
  if (!empty($_FILES['image']['name'])) {
    ...
    if (substr($image_type,0,5)=="image") {
      ...
      if ($type='student') {
        ...
        if ($query_run) {
          ...
          header("Location:../secretary/accounts.php?upsuc&tab-accounts=students");

        }
        else {

          header("location: ../secretary/accounts.php?upsucer&tab-accounts=students");
        }
      }
      elseif ($type='faculty') {
        if ($query_run) {

          header("location:../secretary/accounts.php?upsuc&tab-accounts=faculty");

        }
        else {

          header("Location: ../secretary/accounts.php?upsucer&tab-accounts=faculty");
    ...
    if ($type='faculty') {
      ...
      if ($query_run) {
        header("location: ../secretary/accounts.php?error&tab-accounts=faculty");

      }
      else {
        header("Location:../secretary/accounts.php?suc&tab-accounts=faculty");
