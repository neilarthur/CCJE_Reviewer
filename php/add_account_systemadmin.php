...
if (isset($_POST["save"])) {
	...
	if (substr($image_type,0,5)=="image") {
		...
		else
		{
			...
			if ($category=='student') {
				if ($sql_rows) {
					header("Location: ../system_admin/accounts.php?addsuccess&tab-accounts=students");
				}
				else{
					header("Location: ../system_admin/accounts.php?adderror&tab-accounts=students");
				}
			}
			elseif ($category=='faculty') {
				if ($sql_rows) {
					header("Location: ../system_admin/accounts.php?addsuccess&tab-accounts=faculty");
				}
		        else{

		          header("Location: ../system_admin/accounts.php?adderror&tab-accounts=faculty");
		        }
	    	}
		}
	}
