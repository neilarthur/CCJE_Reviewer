...
$resection = $_GET['sec'];
$pdf ->Cell(190 ,-10, 'Section:'.$resection.'',100,100,'C');
...
$accounts_result = "SELECT * FROM tbl_exam_result,accounts,tbl_pre_question WHERE (tbl_exam_result.acc_id = accounts.acc_id) AND (tbl_exam_result.pre_exam_id = tbl_pre_question.pre_exam_id) AND (tbl_exam_result.pre_exam_id = '{$_GET['id']}') AND (accounts.section='{$_GET['sec']}') AND (tbl_pre_question.prepared_by='{$_SESSION['acc_id']}') ORDER BY accounts.last_name ASC";
